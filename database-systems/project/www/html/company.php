...
<?php
	...
	$query = "SELECT c.objectid 'ID', company_name 'Name',"
			." 		Description, created_date 'Date Created', modified_date 'Date Modified' "
			."FROM Company c";
	...
	// Create the table
	print "<table class='table'><h1>Company<button type=\"button\" class=\"btn btn-default pull-right\" aria-label=\"Right Align\" 
			onclick=\"window.location.href='addCompany.html'\">
	...
	for ($i = 0; $i < $num_rows; $i++) {
		...
		{
			print "<th><a href=\"companyInfo.php?companyId=" .$row[0]. "\">" . $row[$j] . "</a></th>";
		}
		print 	"<th><button type=\"button\" class=\"btn btn-default\" aria-label=\"Right Align\" 
					onclick=\"window.location.href='removeCompany.php?companyId=" .$row[0]."'\">
