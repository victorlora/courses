<?php
	...
	$contactId = $_GET['contactId'] ? $_GET['contactId'] : $userId;
...
<?php
	...
	$query = " SELECT c.objectid, fname, lname, location_id, "
			...
			."		 zip_code, country, uname, password, accountType, isPatron, isSponsor "
			." FROM Contact c"
			." INNER JOIN Location l ON c.location_id = l.objectid"
			." WHERE c.objectid = \"" .$contactId. "\"";
	trim($query);
	...
	$isPatron  = $row[13];
	$isSponsor = $row[14];
	$patronChecked  = ($isPatron)  ? 'checked="checked"' : '';
	$sponsorChecked = ($isSponsor) ? 'checked="checked"' : '';
?>

<section class="container">
	<h4 >Please fill out the information to be updated</h4>
	<form id="form" name="form" method="post" action="updateContactInfo.php">
		<input type="hidden" name="contactId"id="contactId" value="<?php echo $contactId ?>">
		<section class="form-group">
			<label for="userName">Username</label>
			<input type="text" name="userName" id="userName" class="form-control" value="<?php echo $uname ?>">
		</section>
		<section class="form-group">
		...
		<section class="form-group">
			...
			<select name="accountType" id="accountType">
  				<option value="Regular" <?php if ($accountType == "Regular") echo "selected"?>>Regular</option>
  				<option value="Administrator" <?php if ($accountType == "Administrator") echo "selected"?>>Administrator</option>
			</select>
		<hr>
		<section class="form-group">
		...
		<section class="form-group">
			...
			<input type="checkbox" name="sponsor" id="sponsor" value="1" <?php echo $sponsorChecked ?>/>
			<label for="patron">Patron</label>
			<input type="checkbox" name="patron" id="patron" value="1" <?php echo $patronChecked ?>/>
		</section>
		<section class="form-group">
